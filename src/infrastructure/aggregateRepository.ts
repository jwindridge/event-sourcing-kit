...
export interface IAggregateRepository<T> {
  ...
  save(id: string, expectedVersion: number, events: IDomainEvent[]): Promise<IApplicationEvent[]>;
}

export function createAggregateRepository<T> (aggregate: IAggregate<T>, eventStore: IEventStore): IAggregateRepository<T> {
  const getAggregateId = (id: string): IAggregateId => ({ id, name: aggregate.name });
  ...
  const save = async (id: string, expectedVersion: number, events: IDomainEvent[]) => eventStore.saveAll(
    getAggregateId(id),
    expectedVersion,
    events
  )
